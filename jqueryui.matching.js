...
$(function () {
    $.ui.ddmanager.prepareOffsets = function (t, event) {
        let transform = $('.reveal > .slides').css('transform').match(/matrix\(([-.0-9]+), ([-.0-9]+), ([-.0-9]+), ([-.0-9]+), ([-.0-9]+), ([-.0-9]+)\)/);
        let scale = transform ? transform[1] : 1;
        let zoom = $(this).closest('section').css('zoom') ? $(this).closest('section').css('zoom') : 1;
        ...
        droppablesLoop: for (i = 0; i < m.length; i++) {
            ...
            m[i].proportions({
                width: m[i].element[0].offsetWidth * scale * zoom,
                height: m[i].element[0].offsetHeight * scale * zoom
            });
        }
